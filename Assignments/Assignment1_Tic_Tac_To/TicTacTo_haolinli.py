...
% Student Name: Haolin Li

% Student Unique Name: haolinli
...
### QUESTION: why is there a "Nobody" at the front of the list?
...
def make_move(player, board):
    ...
    while True:
        ...
        move_input = input("" + player_name(player) + "'s move: ")
        # Validation #1: the input is a single char and is between 1 and 9
        if len(move_input) != 1 or ord(move_input) > 57 or ord(move_input) < 49:
            print("Please input a VALID number from 1-9")
        # Validation #2: the cell is not occupied
        elif board[int(move_input) - 1] != 0:
            print("This cell is already OCCUPIED!")
        else:
            # else, record the cell and break the loop
            board[int(move_input) - 1] = player
            break
...
def check_win_diagonal(board):
    """check diagonally if there is a winner
...
# make the final move for the player
display_board(board)
if moves_left == 0:
    print("Game Over! There is no winner!")
else:
    print("Game Over! The winner is: " + player_name(winner) + " !")
